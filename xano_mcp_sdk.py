#!/usr/bin/env python3
"""
Xano MCP Server - Local deployment version for Claude Desktop
"""
...
def get_token():
    ...
    for i, arg in enumerate(sys.argv):
        if arg == "--token" and i + 1 < len(sys.argv):
            log_debug(f"Found token in command line arguments")
            return sys.argv[i + 1]
...
# Utility function to get the meta API URL for an instance, prioritizing custom domains
def get_meta_api_url(instance_name: str) -> str:
    """Get the appropriate meta API URL for an instance, prioritizing custom domains."""
    # If instance_name looks like a full URL
    if instance_name.startswith("http://") or instance_name.startswith("https://"):
        return f"{instance_name}/api:meta"
    # If it looks like a domain name
    elif "." in instance_name and "/" not in instance_name:
        return f"https://{instance_name}/api:meta"
    else:
        # Try to get instance details (may be async in actual use, but for this context, call directly)
        try:
            details = xano_get_instance_details(instance_name)
            # If details is a coroutine (async), run it
            if hasattr(details, "__await__"):
                import asyncio
                details = asyncio.get_event_loop().run_until_complete(details)
        except Exception as e:
            log_debug(f"Error getting instance details for custom domain: {e}")
            details = {}
        # Prioritize custom domain
        if details and "custom_domain" in details and details["custom_domain"]:
            return f"https://{details['custom_domain']}/api:meta"
        else:
            instance_domain = details.get("xano_domain", f"{instance_name}.n7c.xano.io")
            return f"https://{instance_domain}/api:meta"
...
async def xano_list_databases(instance_name: str) -> Dict[str, Any]:
    ...
    meta_api = get_meta_api_url(instance_name)
